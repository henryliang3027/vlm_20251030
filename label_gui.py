...
from ministral_test_base import MinistralVLM
...
def update_display():
    ...
    if image_path.exists():
        generated_output = ministralVLM.generate(image_path)
...
with ui.column().classes('w-full p-4'):
    ...
    # Counter
    counter_label = ui.label(f"Image 1 / {len(data)}").classes('text-xl mb-4 text-center')
        ...
        with splitter.after:
            with ui.column().classes('w-full h-full gap-4 p-4'):
                ...
                with ui.card().classes('w-full'):
                    ...
                    question_input = ui.textarea(placeholder='Enter your text...').props('autogrow').classes('w-full')

                # Think section
                with ui.card().classes('w-full'):
                    ui.label('Think:').classes('text-lg font-bold mb-2')
                    think_input = ui.textarea(placeholder='Enter your text...').props('autogrow').classes('w-full')

                # Answer section
                with ui.card().classes('w-full'):
                    ui.label('Answer:').classes('text-lg font-bold mb-2')
                    answer_input = ui.textarea(placeholder='Enter your text...').props('autogrow').classes('w-full')

                # Ministral Generate label
                with ui.card().classes('w-full'):
                    ui.label('Ministral Generate:').classes('text-lg font-bold mb-2')
                    ministral_generate_label = ui.label('').classes('text-base whitespace-pre-wrap')
